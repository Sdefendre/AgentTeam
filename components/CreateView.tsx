...
export default function CreateView() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // The API will handle progress updates via Server-Sent Events or polling
      // For now, we'll use the results directly
      if (result.success) {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {error && (
        <div className="mb-4 p-3 card border-red-800/50 flex items-center gap-3">
          ...
          <p className="flex-1 text-sm text-slate-300">{error}</p>
          <button onClick={clearError} className="text-slate-500 hover:text-red-400">
      ...
      <div className="card p-6">
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors resize-none h-24"
          />
        </div>
